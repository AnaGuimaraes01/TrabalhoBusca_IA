...
class Puzzle:
    ...
    def __eq__(self, other):
        return isinstance(other, Puzzle) and self.estado == other.estado
    ...
    def gerar_sucessores(self) -> List['Puzzle']:
        ...
        for direcao, (condicao, deslocamento) in movimentos.items():
            if condicao:
                ...
                sucessores.append(
                    Puzzle(tuple(novo_estado), self.tamanho, self, direcao, self.profundidade + 1)
                )

        return sucessores

    def caminho(self) -> List[str]:
        """Retorna a sequência de ações desde o início até este estado"""
        acoes = []
        atual = self
    ...
    def caminho_estados(self) -> List[Tuple[int]]:
        """Retorna os estados do caminho desde o estado inicial até este"""
        estados = []
        atual = self
